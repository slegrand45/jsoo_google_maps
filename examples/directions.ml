...
let demo doc =
  let element =
    Js.Opt.get (doc##getElementById(Js.string "demo_directions"))
      (fun () -> assert false)
  in
  let chicago = new%js GM.latLng (Js.number_of_float 41.85) (Js.number_of_float (-87.65)) Js.undefined in
  let indianapolis = new%js GM.latLng (Js.number_of_float 39.79) (Js.number_of_float (-86.14)) Js.undefined in
  ...
  let directions_display = new%js GM.directionsRenderer(Js.def options) in
  ...
  let directions_service = new%js GM.directionsService in
